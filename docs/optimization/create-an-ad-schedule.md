...
On the campaign dashboard, click the desired campaign name to open the line item dashboard. 

On the line item dashboard, click + Line item to create a new line item or click the three dots against your desired line item to edit it 

 In the line item creation workflow, scroll down to find the Ad Schedule tool under the larger Schedule heading in Step 1. 

Click the arrow next and choose Run ads on a schedule to reveal a series of three dropdowns that can be used to select your desired schedule: Day(s) of the week, Start Time, and End Time. 
...
Use the Add button to build out your schedule.
You can also input multiple intraday parameters - for instance, you can add multiple rules for Monday, allowing ads to be served from 7:00-9:00 a.m., later in the day from 12:00-2:00 p.m., and 5:00-End of Day. Ensure that you’ve selected an end time after your start time. If you set conflicting rules, for example –   

Monday: 9:00 a.m.-10:00 p.m.  

Tuesday: 6:00 a.m.-4:00 p.m.  

Weekdays (Monday-Friday): 7:00 a.m.-5:00 p.m.   

The system will take the earliest start time available to each period and the latest possible end time:   
...
Click the x on the right of each rule to remove it. To reset the schedule entirely, click Run ads all the time.
...
Note: Hourly reporting is available in the Analytics tab. Select the Hour dimension in the Activity dashboard to analyze performance by hour.
...
Ad scheduling is more effective when campaigns have sufficient data to identify peak engagement times. If a campaign is struggling to hit daily caps, it may not have gathered enough data to pinpoint the most effective time slots for ad delivery accurately. Implementing dayparting without a clear understanding of when shoppers are most active may lead to missed opportunities or, conversely, overspending during less effective time periods.
